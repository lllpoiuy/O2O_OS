...
class HyperEmbedder(nn.Module):  # [NOTE] Shift + l_2 norm & Linear + Scaler, SimBaV2 / trick-1.1 & trick-1.2
    hidden_dim: int
    scaler_init: float
    scaler_scale: float
    c_shift: float
...
class HyperLERPBlock(nn.Module):  # [NOTE] MLP + l_2 norm & LERP + l_2 norm, SimBaV2 / trick-2.1 & trick-2.2
    hidden_dim: int
    scaler_init: float
    scaler_scale: float
    alpha_init: float
...
class HyperCategoricalValue(nn.Module):  # [NOTE] Distributional Critic, SimBaV2 / trick-3.1
    hidden_dim: int
    num_bins: int
    min_v: float
    max_v: float
